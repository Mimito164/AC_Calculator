...
class Calculator(object):
    ...
    def voltageDivider(self, complexName: str, v: VoltageSource,
                       z1: ComplexNumer, z2: ComplexNumer, verbose=False) -> ComplexNumer:
        if verbose:
            result = ComplexNumer(complexName, v.value *
                                  (z1.value / (z1.value + z2.value)))
            d1 = z1.name + " + " + z2.name
    ...
    def currentDivider(self, complexName: str, i: CurrentSource,
                       z1: ComplexNumer, z2: ComplexNumer, verbose=False) -> ComplexNumer:

        result = ComplexNumer(complexName, i.value *
                              (z2.value / (z1.value + z2.value)))
    ...
    def power(self, complexName: str, v: VoltageSource, i: CurrentSource,
              verbose=False) -> ComplexNumer:
        ...
        if verbose:
            ...
            print(
                f"{result.name} = 1/2 * {v.toExp()} * ~({i.toExp()}) = 1/2 * {v.toExp()} * {iConj.toExp()}")
